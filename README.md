...
Удаление транка с регистрацией или без регистрации принимает два аргумента:

```bash
./pjsip_trunk_upsert provider1 remove
```

Скрипт удалит весь управляемый блок транка: `endpoint`, `aor`, `identify`, а при наличии также `auth` и `registration`. Транки, которые не были созданы этим скриптом и не имеют его маркеров, не удаляются.
